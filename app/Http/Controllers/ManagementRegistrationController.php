...
class ManagementRegistrationController extends Controller
{
    ...
    public function GetApplicantData(Request $request)
    {
        ...
        $sql = "call biodata_getdata('" . ($request['Type'] == null ? '' : $request['Type']) . "', '" . ($request['Category'] == null ? '' : $request['Category']) . "', '" . ($request['Operations'] == null ? '' : $request['Operations']) . "', " . ($request['AgeFrom'] == null ? 0 : $request['AgeFrom']) . ", " . ($request['AgeTo'] == null ? 0 : $request['AgeTo']) . ", '" . ($request['SubmissionDate'] == null ? '' : $request['SubmissionDate']) ."', '" . $sorCol . "', '" . $sorDir . "', " . $start . ", " . $length . ", '" . $search . "')";
        $data = collect(DB::select(DB::raw($sql)));
        $sql = "call biodata_getdata('" . ($request['Type'] == null ? '' : $request['Type']) . "', '" . ($request['Category'] == null ? '' : $request['Category']) . "', '" . ($request['Operations'] == null ? '' : $request['Operations']) . "', " . ($request['AgeFrom'] == null ? 0 : $request['AgeFrom']) . ", " . ($request['AgeTo'] == null ? 0 : $request['AgeTo']) . ", '" . ($request['SubmissionDate'] == null ? '' : $request['SubmissionDate']) ."', '" . $sorCol . "', '" . $sorDir . "', 0, 10000, '" . $search . "')";
        ...
        for ($i = 0; $i < count($request["PersonalID"]); $i++) {
            if ($request["PersonalID"][$i]["Value"] == 1) {
                DB::table('personal_datas')
                    ...
                    ->update([
                        'to_abroad' => 1, 'abroad_date' => $request["PersonalID"][$i]["AbroadDate"]
                    ]);
            } else {
                DB::table('personal_datas')
                    ->where('id', $request["PersonalID"][$i]["ID"])
                    ->update([
                        'to_abroad' => 0, 'abroad_date' => null
                    ]);
            }
        }
        $data = [
